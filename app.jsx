...
const LZ_HAZARDS_ITEMS = [
  ...
  { key: "vehicles", label: "Vehicles Clear" },
...
const LZ_SURFACE_ITEMS = [
  ...
  { key: "minimalSlope", label: "Minimal Slope" },
];
...
function Section({ title, children, accent, glow, fill }) {
  return (
    <div
...
function ChecklistSection({ title, items, data, onToggle, accent, glow }) {
  return (
    <Section title={title} accent={accent} glow={glow}>
      {items.map((it) => (
...
function TacticalWorksheet() {
  ...
  function handleWeightChange(unit, value) {
    setState((s) => {
      const num = parseFloat(value);
      let weightKg = s.lifeFlight.weightKg;
      let weightLbs = s.lifeFlight.weightLbs;
      if (unit === "kg") {
        weightKg = value;
      ...
      return { ...s, lifeFlight: { ...s.lifeFlight, weightKg, weightLbs } };
    });
  }
  ...
  function updateLifeFlightUnit(listKey, id, field, value) {
    setState((s) => {
      const list = s.lifeFlight[listKey].map((u) => (u.id === id ? { ...u, [field]: value } : u));
      const draft = { ...s, lifeFlight: { ...s.lifeFlight, [listKey]: list } };
      const u = list.find((x) => x.id === id);
      if (field === "assign" && value) addLog(draft, `${u.unit || "Unit"} assigned to ${value}`);
      return draft;
    });
  }
  ...
  function removeLifeFlightUnit(listKey, id) {
    setState((s) => ({
      ...s,
      lifeFlight: { ...s.lifeFlight, [listKey]: s.lifeFlight[listKey].filter((u) => u.id !== id) },
    }));
  }
  ...
  function toggleTaskCheck(slotId, key, label) {
    setState((s) => {
      ...
      addLog(draft, `${slot.label}${slot.unit ? ` (${slot.unit})` : ""} — ${label} ${
        slot.checks[key] == null ? "cleared" : "confirmed"
      }`);
      return draft;
    });
  }
  ...
  function toggleTaskSegment(slotId, key, mode, label) {
    setState((s) => {
      ...
      addLog(
        draft,
        `${slot.label}${slot.unit ? ` (${slot.unit})` : ""} — ${label} ${newValue ? `set to ${newValue.mode}` : "cleared"}`
      );
      return draft;
    });
  }
  ...
  function updateEmsAlarmUnit(listKey, id, field, value) {
    setState((s) => {
      const list = s[listKey].map((u) => (u.id === id ? { ...u, [field]: value } : u));
      ...
      if (field === "assign" && value) {
        ...
        addLog(draft, `${u.unit || "EMS unit"} assigned to ${value} — Medical confirmed`);
      }
  ...
  const assignedEmsUnits = [...state.firstAlarmEmsUnits, ...state.secondAlarmEmsUnits]
          ...
          <TextField
            ...
            onChange={(v) =>
              setState((s) => ({
                ...s,
                address: v,
                command: { ...s.command, name: deriveCommandName(v) },
              }))
            }
  ...
  return (
    ...
    >
      {/* Top bar */}
      <div className="flex flex-wrap items-center gap-3 px-4 py-2.5" style={{ background: theme.panel, borderBottom: `1px solid ${theme.border}` }}>
        <BadgeIcon size={24} />
        ...
        <button
          onClick={callPAR}
          className={`px-3 py-1.5 rounded text-xs font-bold ${parDue ? "animate-pulse" : ""}`}
          style={{
        ...
        <div className="flex-1 overflow-y-auto p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              ...
              <Section title="Location">
                ...
                <CheckRow
                  label="Center or End Unit"
                  value={state.sizeUp.centerOrEnd ? 1 : null}
                  onToggle={() => setSizeUp("centerOrEnd", !state.sizeUp.centerOrEnd, `Center or End Unit ${!state.sizeUp.centerOrEnd ? "confirmed" : "cleared"}`)}
                />
              </Section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              ...
              <Section title="Follow-Up">
                <CheckRow label="360° Complete" value={state.followUp.threeSixty} onToggle={() => toggleSimple("followUp", "threeSixty", "360°")} />
                <div className="mb-3">
                ...
                <Section title="Strategy Confirmation">
                  {["Offensive", "Defensive"].map((o) => (
                    <Pill
                      ...
                      color={o === "Offensive" ? theme.red : theme.blue}
                    >
                      {o}
                    </Pill>
                  ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {state.taskSlots.map((slot) => (
                <Section key={slot.id} title={slot.label}>
                  <div className="mb-3">
                    <TextField value={slot.unit} onChange={(v) => setTaskField(slot.id, "unit", v)} placeholder="Unit" />
                  </div>
                  {TASK_CHECKLIST.map((c) => (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              <ChecklistSection title="Utilities" items={UTILITIES_ITEMS} data={state.utilities} onToggle={(k, l) => toggleSimple("utilities", k, l)} />
              ...
              <Section title="Medical / Rehab">
                ...
                {state.medicalRehab.medical != null && (
                  <>
                    ...
                    <div className="mb-1.5">
                      {SIDE_OPTS.map((o) => (
                        <Pill key={o} active={state.medicalRehab.medicalSide === o} onClick={() => setMedicalRehabField("medicalSide", o, `Medical: Side ${o}`)}>
                          {o}
                        </Pill>
                      ))}
                    </div>
                ...
                {state.medicalRehab.rehab != null && (
                  <div className="mb-1.5">
                    {SIDE_OPTS.map((o) => (
                      <Pill key={o} active={state.medicalRehab.rehabSide === o} onClick={() => setMedicalRehabField("rehabSide", o, `Rehab: Side ${o}`)}>
                        {o}
                      </Pill>
                    ))}
                  </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              ...
              <Section title="Patient Information">
                <div className="grid grid-cols-2 gap-2 mb-3">
                  <TextField label="Age" value={state.lifeFlight.patientAge} onChange={(v) => setLifeFlightField("patientAge", v)} placeholder="Age" />
                  <div>
                    <label className="block text-xs uppercase tracking-wide mb-1" style={{ color: theme.textDim }}>
                      Sex
                    </label>
                    {["Male", "Female"].map((o) => (
                      <Pill key={o} active={state.lifeFlight.patientSex === o} onClick={() => setLifeFlightField("patientSex", o, `Patient Sex: ${o}`)}>
                        {o}
                      </Pill>
                    ))}
                  </div>
                ...
                <div className="mb-3">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <TextField value={state.lifeFlight.weightKg} onChange={(v) => handleWeightChange("kg", v)} placeholder="kgs" />
                    <TextField value={state.lifeFlight.weightLbs} onChange={(v) => handleWeightChange("lbs", v)} placeholder="lbs" />
                  </div>
                </div>

                <TextField
                  label="Problem Description"
                  value={state.lifeFlight.patientProblemDescription}
                  onChange={(v) => setLifeFlightField("patientProblemDescription", v)}
                  placeholder="Problem Description"
                  className="mb-3"
                />
                ...
                <div>
                  ...
                  {PATIENT_INTERVENTION_ITEMS.map((it) => (
                    <CheckRow
                      key={it.key}
                      label={it.label}
                      value={state.lifeFlight.interventions[it.key]}
                      onToggle={() => toggleLifeFlightNested("interventions", it.key, it.label)}
                    />
                  ))}
                  {state.lifeFlight.interventions.other != null && (
                    <TextField
                      className="mt-1"
                      value={state.lifeFlight.interventionOther}
                      onChange={(v) => setLifeFlightField("interventionOther", v)}
          ...
          {tab === "mayday" && (
            <div className="flex flex-col gap-4 max-w-2xl">
              <Section title="1. Initial Actions" accent={theme.red}>
                <CheckRow
                  label="Revert to SEMS, verify unit in Alarm, their Air Supply, and document Radio Identifier via Mobile Radio"
                  value={state.mayday.step1}
                  onToggle={() => toggleSimple("mayday", "step1", "Step 1: Revert to SEMS / verify unit / air supply / radio ID")}
                />
                <TextField
                  className="mt-1"
                  value={state.mayday.step1Unit}
                  onChange={(v) => setMaydayField("step1Unit", v)}
                  placeholder="Unit"
                />
              </Section>

              <Section title="2. Transmit" accent={theme.red}>
                <CheckRow
                  label={`"All units clear this channel for a MAYDAY; unit calling MAYDAY go ahead with your CAN Report."`}
                  value={state.mayday.step2}
                  onToggle={() => toggleSimple("mayday", "step2", "Step 2: Transmit — clear channel for MAYDAY")}
                />
              </Section>

              <Section title="3. Listen & Document" accent={theme.red}>
                <CheckRow
                ...
                <div className="mt-3 mb-1">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <TextField value={state.mayday.whoUnit} onChange={(v) => setMaydayField("whoUnit", v)} placeholder="Unit" />
                  </div>
                </div>
              ...
              <Section title="4. Transmit" accent={theme.red}>
                <CheckRow
              ...
              <Section title="5. Deploy" accent={theme.red}>
                <CheckRow
              ...
              <Section title="6. Transmit" accent={theme.red}>
                <CheckRow
              ...
              <Section title="7. CAN Report" accent={theme.red}>
                <CheckRow
              ...
              <Section title="8. Transmit" accent={theme.red}>
                <CheckRow
                  label={`"Command to all units, there is a Mayday in progress, (Who, What, Where)."`}
                  value={state.mayday.step8}
                  onToggle={() => toggleSimple("mayday", "step8", "Step 8: Transmit — Mayday in progress to all units")}
                />
              </Section>

              <Section title="9. Assign" accent={theme.red}>
                <CheckRow
                  label="Assign additional companies to RIT & Division Officers"
                  value={state.mayday.step9}
                  onToggle={() => toggleSimple("mayday", "step9", "Step 9: Assign additional companies to RIT & Division Officers")}
                />
              </Section>

              <Section title="10. Clear" accent={theme.red}>
                <CheckRow
